...
namespace CoreNs
{
	...
	private:
		...
		ParaValue activated_; // Check if fault is activated in parallel pattern fault simulation
		...
		std::vector<std::array<ParaValue, 5>> faultInjectLow_;
		std::vector<std::array<ParaValue, 5>> faultInjectHigh_;
	...
	inline Simulator::Simulator(Circuit *pCircuit)
			: pCircuit_(pCircuit),
				...
				processed_(pCircuit->totalGate_, false),
				recoverGates_(pCircuit->totalGate_),
				faultInjectLow_(pCircuit->totalGate_, std::array<ParaValue, 5>({0, 0, 0, 0, 0})),
				faultInjectHigh_(pCircuit->totalGate_, std::array<ParaValue, 5>({0, 0, 0, 0, 0}))
	{
	}
	...
	inline void Simulator::goodValueEvaluation(const int &gateID)
	{
		...
		// read value of fanin
		const ParaValue &l1 = pCircuit_->circuitGates_[fanin1].goodSimLow_;
		const ParaValue &h1 = pCircuit_->circuitGates_[fanin1].goodSimHigh_;
		const ParaValue &l2 = pCircuit_->circuitGates_[fanin2].goodSimLow_;
		const ParaValue &h2 = pCircuit_->circuitGates_[fanin2].goodSimHigh_;
		const ParaValue &l3 = pCircuit_->circuitGates_[fanin3].goodSimLow_;
		const ParaValue &h3 = pCircuit_->circuitGates_[fanin3].goodSimHigh_;
		const ParaValue &l4 = pCircuit_->circuitGates_[fanin4].goodSimLow_;
		const ParaValue &h4 = pCircuit_->circuitGates_[fanin4].goodSimHigh_;
	...
	inline void Simulator::faultValueEvaluation(const int &gateID)
	{
		...
		const ParaValue l1 = (pCircuit_->circuitGates_[fanin1].faultSimLow_ & ~faultInjectHigh_[gateID][1]) | faultInjectLow_[gateID][1];
		const ParaValue h1 = (pCircuit_->circuitGates_[fanin1].faultSimHigh_ & ~faultInjectLow_[gateID][1]) | faultInjectHigh_[gateID][1];
		const ParaValue l2 = (pCircuit_->circuitGates_[fanin2].faultSimLow_ & ~faultInjectHigh_[gateID][2]) | faultInjectLow_[gateID][2];
		const ParaValue h2 = (pCircuit_->circuitGates_[fanin2].faultSimHigh_ & ~faultInjectLow_[gateID][2]) | faultInjectHigh_[gateID][2];
		const ParaValue l3 = (pCircuit_->circuitGates_[fanin3].faultSimLow_ & ~faultInjectHigh_[gateID][3]) | faultInjectLow_[gateID][3];
		const ParaValue h3 = (pCircuit_->circuitGates_[fanin3].faultSimHigh_ & ~faultInjectLow_[gateID][3]) | faultInjectHigh_[gateID][3];
		const ParaValue l4 = (pCircuit_->circuitGates_[fanin4].faultSimLow_ & ~faultInjectHigh_[gateID][4]) | faultInjectLow_[gateID][4];
		const ParaValue h4 = (pCircuit_->circuitGates_[fanin4].faultSimHigh_ & ~faultInjectLow_[gateID][4]) | faultInjectHigh_[gateID][4];
