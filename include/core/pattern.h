...
inline void PatternProcessor::StaticCompression()
{
    ...
    for(int i=0; i<size; ++i){mergeRecord[i]=false;}
    bool* patternTable = new bool[size*size]();
    for (int i=0; i<size; ++i){patternTable[i*size+i]= true;}
...
inline bool PatternProcessor::updatetable(bool* mergeRecord, bool* patternTable){
    ...
    while(pattern_candidate.size()!=0){
        ...
        for(int i = 0; i <size; ++i){update_similarity_pattern[i] = false;}
        ...
        if (max_similarity == 0){return false;}
        ...
        for(int i =0; i <size; ++i){
            ...
            if(similarity_pattern_second==i){continue;}
            if(patternTable[similarity_pattern_second*size+i]){del_second_pattern.push_back(i);}
        }
        ...
        */
        ...
        for(int i = 0; i <size; ++i){
            ...
            if(update_similarity_pattern[i]){
                for(int j = 0; j < size; ++j){
                    if(i==j){continue;}
                    if(mergeRecord[j]){continue;}
                    if(patternTable[i*size+j]){
                        Pattern_pair temp_pair;
                        if(i<j){temp_pair.first=i; temp_pair.second = j;}
                        else{temp_pair.first=j; temp_pair.second = i;}
                        Pattern_table::iterator it = pattern_candidate.find(temp_pair);
                        if(it!=pattern_candidate.end()){
                            int count_similarity = 0;
                            for(int b = 0; b<size;++b){
                                if(mergeRecord[b]){continue;}
                                if(patternTable[i*size+b] && patternTable[i*size+b]==patternTable[j*size+b]){++count_similarity;}
                            }
                            it->second = count_similarity;
                        }
                    }
