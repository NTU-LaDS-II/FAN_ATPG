...
bool Netlist::check(const bool &verbose) const {
    ...
    for (size_t i = 0; i < top_->getNNet(); ++i) {
        ...
        for ( ; it != eqvs.end(); ++it) {
            ...
            for (size_t k = 0; k < eqv->getNPort(); ++k) {
                ...
                if ((p->top_ != top_ && p->type_ == Port::OUTPUT)
                    || (p->top_ == top_ && p->type_ == Port::INPUT))
                    hasIn = true;
                if ((p->top_ != top_ && p->type_ == Port::INPUT)
                    || (p->top_ == top_ && p->type_ == Port::OUTPUT))
                    hasOut = true;
                if ((p->top_ != eqv->top_ && p->exNet_ != eqv)
                     || (p->top_ == eqv->top_ && p->inNet_ != eqv)) {
                    res = false;
                    if (verbose) {
                        fprintf(stderr, "**ERROR Netlist::check(): net ");
                        fprintf(stderr, "`%s/%s' ", top_->name_, n->name_);
    ...
    for (size_t i = 0; i < top_->getNCell(); ++i) {
        ...
        if (c->libc_) {
            if (lib_->hasPmt(c->libc_->id_, Pmt::TSL)
                || lib_->hasPmt(c->libc_->id_, Pmt::TSLI)
                || lib_->hasPmt(c->libc_->id_, Pmt::TSH)
                || lib_->hasPmt(c->libc_->id_, Pmt::TSHI)) {
                res = false;
                if (verbose) {
            ...
            if (lib_->hasPmt(c->libc_->id_, Pmt::NMOS)
                || lib_->hasPmt(c->libc_->id_, Pmt::PMOS)
                || lib_->hasPmt(c->libc_->id_, Pmt::RNMOS)
                || lib_->hasPmt(c->libc_->id_, Pmt::RPMOS)
                || lib_->hasPmt(c->libc_->id_, Pmt::NMOSF)
                || lib_->hasPmt(c->libc_->id_, Pmt::PMOSF)
                || lib_->hasPmt(c->libc_->id_, Pmt::CMOS1)
                || lib_->hasPmt(c->libc_->id_, Pmt::CMOS2)
                || lib_->hasPmt(c->libc_->id_, Pmt::RCMOS1)
                || lib_->hasPmt(c->libc_->id_, Pmt::RCMOS2)
                || lib_->hasPmt(c->libc_->id_, Pmt::CMOS1F)
                || lib_->hasPmt(c->libc_->id_, Pmt::CMOS2F)) {
                res = false;
                if (verbose) {
                    fprintf(stderr, "**ERROR Netlist::check(): cell ");
                    fprintf(stderr, "`%s/%s' ", top_->name_, c->name_);
            ...
            if (lib_->hasPmt(c->libc_->id_, Pmt::TIE1)
                || lib_->hasPmt(c->libc_->id_, Pmt::TIE0)
                || lib_->hasPmt(c->libc_->id_, Pmt::TIEX)
                || lib_->hasPmt(c->libc_->id_, Pmt::TIEZ)) {
                if (verbose) {
                    fprintf(stderr, "**WARN Netlist::check(): cell ");
                    fprintf(stderr, "`%s/%s' ", top_->name_, c->name_);
