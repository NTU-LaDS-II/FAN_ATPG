...
bool AddFaultCmd::addPinFault(const string &type, const string &pin)
{
	...
	Fault *f = &fanMgr_->fListExtract->uncollapsedFaults_[fid];
...
bool ReportStatsCmd::exec(const vector<string> &argv)
{
	...
	for (; it != fanMgr_->fListExtract->faultsInCircuit_.end(); ++it)
	{
		...
		{
			case Fault::UD:
				// cout << (*it)->gateID_ << ' ' << (*it)->faultyLine_ << ' ' << (*it)->faultType_ << ' ' << (((*it)->faultyLine_==0)?(0):(fanMgr_->cir->gates_[(*it)->gateID_].fis_[(*it)->faultyLine_-1])) << endl;
				ud += eq;
				break;
			case Fault::DT:
				dt += eq;
