...
int main(int argc, char **argv) {
    ...
    // interactive shell
    cout << "** This is the common package test binary" << endl;
    cout << "** Please report any bugs to littleshamoo. Thanks" << endl;
    ...
    if (!tmUsg.getTotalUsage(tmEnd))
        cerr << "**ERROR main(): cannot get resource usage" << endl;
    cout << "# Runtime and memory usage" << endl;
    cout << "# user time: " << (double)tmEnd.uTime / 1000000.0 << "s" <<endl;
    cout << "# sys time:  " << (double)tmEnd.sTime / 1000000.0 << "s" <<endl;
    cout << "# real time: " << (double)tmEnd.rTime / 1000000.0 << "s" <<endl;
    cout << "# cur mem:   " << (double)tmEnd.vmSize / 1024.0 << "MB" << endl;
    cout << "# peak mem:  " << (double)tmEnd.vmPeak / 1024.0 << "MB" << endl;
    cout << "# diff mem:  " << (double)tmEnd.vmDiff / 1024.0 << "MB" << endl;
    cout << endl;
    cout << "** Goodbye" << endl;
...
public:
         TestLargeLoopCmd(const string &name);
         ~TestLargeLoopCmd();

    bool exec(const vector<string> &argv);
};

TestLargeLoopCmd::TestLargeLoopCmd(const string &name) : Cmd(name) {
...
bool TestLargeLoopCmd::exec(const vector<string> &argv) {
    optMgr_.parse(argv);
    int size = optMgr_.getNParsedArg() > 0 ?
                 atoi(optMgr_.getParsedArg(0).c_str()) : 1500;
