...
public:
    ...
    enum GENERATION_STATUS     { TEST_FOUND = 0, UNTESTABLE, ABORT };
    enum LINE_TYPE             { FREE_LINE  = 0, HEAD_LINE,  BOUND_LINE };
    enum XPATH_STATE           { NO_X_EXIST = 0, UNKNOWN,    X_EXIST };
    enum IMP_STATUS            { FORWARD = 0, BACKWARD,CONFLICT };
    enum BACKTRACE_STATUS      { INITIAL = 0, CHECK_AND_SELECT,
                                 CURROBJ_DETERMINE, FANOBJ_DETERMINE };
    enum BACKTRACE_RESULT      { NO_CONTRADICTORY  = 0, CONTRADICTORY };
    enum FIND_OBJECTIVE_STATUS { CHECK_UNJUSTIFIED = 0, BACKTRACE_CURR,
                                 BACKTRACE_FANOUT     , CHECK_HEADOBJ };
    enum MAIN_STATUS           { IMPLY_AND_CHECK   = 0, DECISION,BACKTRACK,
                                 JUSTIFY_FREE };
	    ...
	    case Gate::XOR3:
	      ...
	      if( currentFault_.line_ == 0 ){
	         ...
	         if( g.type_ == Gate::XOR2 )
	            val = cXOR2( cir_->gates_[g.fis_[0]].v_
	                       , cir_->gates_[g.fis_[1]].v_ );
	         else
	            val = cXOR3( cir_->gates_[g.fis_[0]].v_
	                       , cir_->gates_[g.fis_[1]].v_
	                       , cir_->gates_[g.fis_[2]].v_ );
	         ...
	         if( g.type_ == Gate::XOR2 ){
	            ...
	            if( FaultyLine-1 == 0 )
	               val = cXOR3( cir_->gates_[g.fis_[1]].v_
	                          , cir_->gates_[g.fis_[2]].v_ , val );
	            else if( FaultyLine-1 == 1 )
	               val = cXOR3( cir_->gates_[g.fis_[0]].v_
	                          , cir_->gates_[g.fis_[2]].v_ , val );
	            else
	               val = cXOR3( cir_->gates_[g.fis_[1]].v_
	                          , cir_->gates_[g.fis_[0]].v_ , val );
	         }
	      }
	      return val;
	   case Gate::XNOR2 :
	   case Gate::XNOR3 :
	      ...
	      if( currentFault_.line_ == 0 ){
	         ...
	         if( g.type_ == Gate::XNOR2 )
	            val = cXNOR2( cir_->gates_[g.fis_[0]].v_
	                        , cir_->gates_[g.fis_[1]].v_ );
	         else
	            val = cXNOR3( cir_->gates_[g.fis_[0]].v_
	                        , cir_->gates_[g.fis_[1]].v_
	                        , cir_->gates_[g.fis_[2]].v_ );
	         ...
	         if( g.type_ == Gate::XNOR2 ){
	            ...
	            if( FaultyLine-1 == 0 )
	               val = cXNOR3( cir_->gates_[g.fis_[1]].v_
	                           , cir_->gates_[g.fis_[2]].v_ , val );
	            else if( FaultyLine-1 == 1 )
	               val = cXNOR3( cir_->gates_[g.fis_[0]].v_
	                           , cir_->gates_[g.fis_[2]].v_ , val );
