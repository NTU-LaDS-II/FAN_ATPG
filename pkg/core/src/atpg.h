	...
	inline Value Atpg::evaluationFaulty(Gate &g)
	{
		...
		switch (g.type_)
		{
			...
			case Gate::BUF:
				...
				if (val == L && (currentFault_.faultType_ == Fault::SA1 || currentFault_.faultType_ == Fault::STF))
					val = B; // logic D' (0/1)
				if (val == H && (currentFault_.faultType_ == Fault::SA0 || currentFault_.faultType_ == Fault::STR))
					val = D; // logic D  (1/0)
				return val;
			case Gate::INV:
				val = cir_->gates_[g.fis_[0]].v_;
				if (currentFault_.faultyLine_ == 0)
				{
					val = cINV(val);
					if (val == L && (currentFault_.faultType_ == Fault::SA1 || currentFault_.faultType_ == Fault::STF))
						val = B;
			...
			case Gate::AND3:
			case Gate::AND4:
				FaultyLine = currentFault_.faultyLine_;
				if (currentFault_.faultyLine_ == 0)
				{
					val = cir_->gates_[g.fis_[0]].v_;
					for (i = 1; i < g.nfi_; i++)
