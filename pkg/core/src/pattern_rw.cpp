...
bool PatternWriter::writeLht(const char *const fname)
{
	...
	for (size_t i = 0; i < pcoll_->pats_.size(); ++i)
	{
		...
		fprintf(fout, " | ");
		if (pcoll_->pats_[i]->po1_ && pcoll_->type_ == PatternProcessor::BASIC_SCAN)
		{
			for (int j = 0; j < pcoll_->npo_; ++j)
			{
				if (pcoll_->pats_[i]->po1_[j] == L)
		...
		if (pcoll_->pats_[i]->po2_ && (pcoll_->type_ == PatternProcessor::LAUNCH_CAPTURE || pcoll_->type_ == PatternProcessor::LAUNCH_SHIFT))
		{
			for (int j = 0; j < pcoll_->npo_; ++j)
			{
				if (pcoll_->pats_[i]->po2_[j] == L)
