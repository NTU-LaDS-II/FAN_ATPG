...
void Simulator::parallelFaultReset()
{
	...
	std::fill(faultInjectLow_.begin(), faultInjectLow_.end(), std::array<ParaValue, 5>({0, 0, 0, 0, 0}));
	std::fill(faultInjectHigh_.begin(), faultInjectHigh_.end(), std::array<ParaValue, 5>({0, 0, 0, 0, 0}));
...
bool Simulator::parallelFaultCheckActivation(const Fault *const pfault)
{
	...
	const ParaValue &faultyGateGoodSimLow = pCircuit_->circuitGates_[faultyGate].goodSimLow_;
	const ParaValue &faultyGateGoodSimHigh = pCircuit_->circuitGates_[faultyGate].goodSimHigh_;
...
{
	const int &faultyGate = pfault->faultyLine_ == 0 ? pfault->gateID_ : pCircuit_->circuitGates_[pfault->gateID_].faninVector_[pfault->faultyLine_ - 1];
	/// if output fault,faultyGate=ID of the faulty gate,else if input fault,faultyGate=ID of the faulty gate's fanin array
	const ParaValue &faultyGateGoodSimLow = pCircuit_->circuitGates_[faultyGate].goodSimLow_;
	const ParaValue &faultyGateGoodSimHigh = pCircuit_->circuitGates_[faultyGate].goodSimHigh_;
