...
void Atpg::TransitionDelayFaultATPG( FaultList& faultListToGen, PatternProcessor* pcoll, int& untest )
{
  ...
  GENERATION_STATUS result = patternGeneration(Fault(fTDF.gate_+cir_->ngate_,fTDF.type_,fTDF.line_,fTDF.eq_,fTDF.state_,fTDF.aggr_), false);
...
void Atpg::identifyLineParameter() {
  ...
  for(int i = 0 ; i < cir_->tgate_ ; i++ ){
    ...
    if( lineType_[g.id_] == FREE_LINE && g.nfo_ != 1){//g.nfo_  number of fanout
...
void Atpg::identifyDominator(){
  for (int i = cir_->tgate_ - 1 ; i >= 0 ; i-- ){
    ...
    for (int j = g.lvl_+ 1 ; j < cir_->tlvl_ ; j++ )
      while(!eventList_[j].empty()){    // if next level's output isn't empty
        ...
        if (gateCount <= 0)
          continue;
        gateCount--;
        if (gateCount == 0){        // when all the fanout gate has been calculated
...
void Atpg::identifyUniquePath() {
  for(int i = cir_->tgate_ - 1 ; i >= 0 ; i-- ){
    ...
    for (int j = g.lvl_ + 1 ; j < cir_->tlvl_ ; j++)
      while(!eventList_[j].empty()){     // if fanout gate has not empty
...
bool Atpg::checkUnjustifiedBoundLines(){
  ...
  for (int i = 0 ; i < (int)unjustified_.size() ; i ++){
    ...
    if (pGate->v_ != X
        && !modify_[pGate->id_]
        && lineType_[pGate->id_] == BOUND_LINE) { // unjustified bound line
      return true;
    }
  }
  return false;
...
void Atpg::assignValueToFinalObject(){
  while (!finalObject_.empty()) {//while exist any finalObject
...
int Atpg::setFaultyGate(Fault& fault)
{
  ...
  if (!isOutputFault){
    ...
    if (pFaultyGate->type_ == Gate::AND2 || pFaultyGate->type_
        == Gate::AND3 || pFaultyGate->type_ == Gate::AND4
        ||  pFaultyGate->type_ == Gate::NAND2 || pFaultyGate->type_
        == Gate::NAND3 || pFaultyGate->type_ == Gate::NAND4
        ||  pFaultyGate->type_ == Gate::NOR2 || pFaultyGate->type_
        == Gate::NOR3 || pFaultyGate->type_ == Gate::NOR4
        ||  pFaultyGate->type_ == Gate::OR2 || pFaultyGate->type_
        == Gate::OR3 || pFaultyGate->type_ == Gate::OR4  ) {
      ...
      for (i = 0 ; i < pFaultyGate->nfi_ ; i ++){
        ...
        if (pFaninGate != pFaultyLine){
          ...
          else if ( pFaninGate->v_
              != pFaultyGate->getInputNonCtrlValue() )
            return -1;
        } else {
          if (isFaultyGateScanned == false) isFaultyGateScanned = true;
          else return -1; // UNTESTABLE FAULT
        }
      }
    ...
    else if (pFaultyGate->type_ == Gate::INV
        || pFaultyGate->type_ == Gate::BUF
        || pFaultyGate->type_ == Gate::PO
        || pFaultyGate->type_ == Gate::PPO ) {
      vtemp = pFaultyGate->isInverse();
      pFaultyGate->v_ = cXOR2(vtemp,FaultyValue);
      backtrackList_.push_back(pFaultyGate->id_);
    }
    ...
    if ( (FaultyValue == D && pFaultyGate->v_ == L)
        ||(FaultyValue == B && pFaultyGate->v_ == H) )
      return -1;
    ...
    else if ((FaultyValue == D
          && pFaultyGate->getOutputCtrlValue() == H )
        ||  (FaultyValue == B
          && pFaultyGate->getOutputCtrlValue() == L ) ) {
...
// **************************************************************************
int Atpg::firstTimeFrameSetUp(Fault& fault){// NE
  Gate   *pFaultyGate;
  Gate   *pFaultyLine;
  int     BackImpLevel = 0;
  bool    isOutputFault = (fault.line_ == 0);
  ...
  if(lineType_[pFaultyLine->id_] == FREE_LINE){
    if ( (FaultyValue == H && pFaultyLine->v_ == L)
        ||(FaultyValue == L && pFaultyLine->v_ == H) )
      return -1;
    ...
    do{
      ...
      if(!gNext->isUnary()&&(gNext->getOutputCtrlValue()==X||gTemp->v_ != gNext->getInputCtrlValue()))break;
    ...
    else if (pFaultyLine->type_ == Gate::INV
        || pFaultyLine->type_ == Gate::BUF
        || pFaultyLine->type_ == Gate::PO
        || pFaultyLine->type_ == Gate::PPO ) {
    ...
    else if ((FaultyValue == H
          && pFaultyLine->getOutputCtrlValue() == H )
        ||  (FaultyValue == L
          && pFaultyLine->getOutputCtrlValue() == L ) ) {
...
int Atpg::uniquePathSensitize(Gate& gate){
  ...
  if (pFaultyGate->id_ != gate.id_){    // if gate is not the FaultyGate
    Value NonControlVal = gate.getInputNonCtrlValue();
    if (NonControlVal != X){          // if gate has an NonControlVal (1 or 0)
    ...
    if (NonControlVal != X && !pNextGate->isUnary() ) {   // If pNextGate is Unary and its NonControlVal is not unknown.
      if (pCurrGate->nfo_ == 1){
        ...
        for (i = 0 ; i < pNextGate->nfi_ ; i ++){
          ...
          if (pFaninGate != pCurrGate
              && pNextGate->getInputCtrlValue() != X
              && pFaninGate->v_
              == pNextGate->getInputCtrlValue() )
            return UNIQUE_PATH_SENSITIZE_FAIL;
...
void Atpg::justifyFreeLines(Fault& fOriginalFault)
{
  ...
  for (i = 0 ; i < nHeadLine_ ; i ++){
    ...
    if (pHeadLineFaultGate == pGate){     // if the HEADLINE scaned now(pGate) is the new faulty gate
...
void Atpg::restoreFault(Fault& fOriginalFault)
{
  ...
  for (i = 0 ; i < pFaultPropGate->nfi_ ; i ++){
    ...
    if ( pFaninGate->v_ == D ) pFaninGate->v_ = H ;
    if ( pFaninGate->v_ == B ) pFaninGate->v_ = L ;
  ...
  while(!fanoutObjective_.empty()){
    ...
    if (!(pGate->type_ == Gate::PI
          || pGate->type_ == Gate::PPI
          || pGate->v_ == X))             // if the gate's value not unknown and the gates type not PI or PPI
...
Atpg::IMP_STATUS Atpg::faultyGateEvaluation(Gate *pGate){
  ...
  else if (Val == pGate->v_){ // The init. faulty objective has already been achieved
    modify_[pGate->id_] = true;
  }
  else if (pGate->v_ == X) {
...
Atpg::IMP_STATUS Atpg::backwardImplication(Gate *pGate){
  ...
  if ( pGate->type_ == Gate::BUF || pGate->type_ == Gate::INV
      || pGate->type_ == Gate::PO  || pGate->type_ == Gate::PPO ){
...
Gate* Atpg::findEasiestInput(Gate* pGate,Value Val){
  ...
  //i = pGate->nfi_;
  //return &cir_->gates_[ pGate->fis_[rand()%i]];
}
  ...
  while(true){
    ...
    switch (atpgStatus){
      ...
      case CURROBJ_DETERMINE:
        ...
        if (lineType_[pCurrentObj->id_] == HEAD_LINE){ //YES
          ...
          for (int i = 0 ; i < pCurrentObj->nfi_ ; i ++){
            ...
            if (pFaninGate->nfo_ > 1){ //YES
              ...
              if (nn0 > 0 || nn1 > 0){
                ...
                if (n0_[pFaninGate->id_] == 0
                    && n1_[pFaninGate->id_] == 0)
                  fanoutObjective_.push_back(pFaninGate->id_);
                ...
                // branch's no, n1
                unsigned NewZero
                  = n0_[pFaninGate->id_] + nn0;
                unsigned NewOne
                  = n1_[pFaninGate->id_] + nn1;
      ...
      case FANOBJ_DETERMINE:
        ...
        if (faultReach_[pCurrentObj->id_] == 1){ //YES
          atpgStatus = CURROBJ_DETERMINE;
          break;//switch break
        }

        //if one of numOfZero or numOfOne is equal to 0
        if ( !(n0_[pCurrentObj->id_] != 0
              && n1_[pCurrentObj->id_] != 0) ) {
          atpgStatus = CURROBJ_DETERMINE;
          break;//switch break
        }
...
bool Atpg::xPathTracing(Gate* pGate)
{
  ...
  for (i = 0 ; i < pGate->nfo_ ; i ++){
    // TO-DO homework 03
    xPathTracing(&(cir_->gates_[pGate->fos_[i]]));
    // Should use if( xPathTracing(&(cir_->gates_[pGate->fos_[i]])) )
    if (xPathStatus_[pGate->fos_[i]] == X_EXIST){
        xPathStatus_[pGate->id_] = X_EXIST;
        return true;
    }
    // end of TO-DO
