...
void Atpg::generation(PatternProcessor *pcoll, FaultListExtract *fListExtract)
{
	...
	while (faultListToGen.begin() != faultListToGen.end())
	{
		...
		if (currF == faultListToGen.front())
		{
			...
			// cout << faultListToGen.front()->gateID_ << ' ' << faultListToGen.front()->faultyLine_ << ' ' << faultListToGen.front()->type_ << ' '
			// 	 << ((faultListToGen.front()->faultyLine_==0)?(0):(cir_->gates_[faultListToGen.front()->gateID_].fis_[faultListToGen.front()->faultyLine_-1])) << endl;
...
void Atpg::TransitionDelayFaultATPG(FaultList &faultListToGen, PatternProcessor *pcoll, int &untest)
{
	...
	GENERATION_STATUS result = patternGeneration(Fault(fTDF.gateID_ + cir_->ngate_, fTDF.faultType_, fTDF.faultyLine_, fTDF.equivalent_, fTDF.faultState_), false);
...
Gate *Atpg::initialize(Fault &fault, int &BackImpLevel, IMP_STATUS &implyStatus, bool isDTC)
{
	...
	Gate *gFaultyLine = &cir_->gates_[fault.gateID_];
	...
	// headLineFault_.gateID_ = -1; //bug report
	if (lineType_[gFaultyLine->id_] == FREE_LINE)
	{
		...
		BackImpLevel = 0;
		implyStatus = FORWARD;
		fGate_id = headLineFault_.gateID_;
	}
...
{ // NE
	...
	pFaultyGate = &cir_->gates_[fault.gateID_ - cir_->ngate_];
...
int Atpg::uniquePathSensitize(Gate &gate)
{
	int i, j;
	int BackImpLevel = -1;
	Gate *pFaultyGate = &cir_->gates_[currentFault_.gateID_];
...
void Atpg::justifyFreeLines(Fault &fOriginalFault)
{
	...
	int gid = headLineFault_.gateID_;
	if (gid != -1)
...
void Atpg::restoreFault(Fault &fOriginalFault)
{
	...
	fanoutObjective_.clear();
	pFaultPropGate = &cir_->gates_[fOriginalFault.gateID_];
...
Atpg::IMP_STATUS Atpg::evaluation(Gate *pGate)
{
	...
	if (pGate->id_ == currentFault_.gateID_)
